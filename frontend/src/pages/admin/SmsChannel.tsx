...
import { smsApi, ApiError, SmsMessageMetadata } from "@/lib/api";
import { useToast } from "@/hooks/use-toast";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { Card } from "@/components/ui/card";
import { Textarea } from "@/components/ui/textarea";
import { ChevronLeft, Phone, MessageSquarePlus } from "lucide-react";
import { useIsMobile } from "@/hooks/use-mobile";
import { cn } from "@/lib/utils";

interface SmsThread extends SmsMessageMetadata {}

interface SmsMessage extends SmsMessageMetadata {}
...
export default function SmsChannel() {
  ...
  const loadThreads = async () => {
    try {
      ...
      const response = await smsApi.listThreads();
      setThreads(response.threads);
    } catch (err) {
      handleApiError(err, "Failed to load SMS threads");
    } finally {
  ...
  const loadMessages = async (threadId: string) => {
    try {
      ...
      const response = await smsApi.getThreadMessages(threadId);
      setMessages(response.messages);
    } catch (err) {
      handleApiError(err, "Failed to load SMS thread messages");
    } finally {
  ...
  const handleCreateTestMessage = async () => {
    try {
      ...
      const message = await smsApi.createTestInbound();
      toast({
        title: "Test SMS created",
        description: "A mock inbound SMS message has been created for testing.",
      });
      await loadThreads();
      setSelectedThreadId(message.metadata?.sms_thread_id || null);
      if (message.metadata?.sms_thread_id) {
        await loadMessages(message.metadata.sms_thread_id);
      }
    } catch (err) {
      handleApiError(err, "Failed to create test SMS message");
    } finally {
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const response = await smsApi.replyToThread(selectedThreadId, replyBody.trim());
      setMessages((prev) => [...prev, response.message]);
      setReplyBody("");
      toast({
        title: "Reply sent",
        description: "Your SMS reply has been sent.",
      });
    } catch (err) {
      handleApiError(err, "Failed to send SMS reply");
    } finally {
        ...
        {isMobile && selectedThreadId ? (
          <>
            ...
            <p className="truncate text-sm font-medium text-foreground flex-1 min-w-0">
              {selectedThread?.content || "SMS Conversation"}
            </p>
          ...
          <>
            ...
            >
              ...
              {creatingTest ? "Creating…" : "Add test SMS"}
            </Button>
          </>
        )}
      </div>
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
            {loadingThreads ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                <p className="text-[13px] text-muted-foreground">No SMS threads yet.</p>
                <p className="text-[12px] text-muted-foreground/80 mt-1">
                  Use &quot;Add test SMS&quot; to create a sample conversation.
                </p>
                <Button
                  onClick={handleCreateTestMessage}
                  disabled={creatingTest}
                ...
                {threads.map((thread) => (
                  ...
                  >
                    <div className="font-medium text-[13px] sm:text-sm truncate pr-2">
                      {thread.content || "SMS conversation"}
                    </div>
          ...
          {!selectedThreadId ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-8 text-center min-h-[280px]">
              <div className="rounded-full bg-muted/50 p-4 mb-4">
                <Phone className="h-8 w-8 text-muted-foreground/70" aria-hidden />
              </div>
              ...
              <p className="text-[13px] text-muted-foreground mt-1.5 max-w-[260px]">
                Choose a thread from the list to view the SMS conversation and reply.
              </p>
            </div>
          ) : loadingMessages ? (
            ...
            <>
              ...
              <div className="shrink-0 p-4 sm:p-5 pt-0 border-t border-border/50 space-y-3">
                <label htmlFor="sms-reply" className="sr-only">
                  Reply
                </label>
                <Textarea
                  id="sms-reply"
                ...
                <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
                  <p className="text-[12px] text-muted-foreground truncate px-0.5">
                    Reply will be sent as a standard SMS message.
                  </p>
                  <Button
