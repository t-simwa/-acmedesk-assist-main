...
import { messengerApi, ApiError, MessengerMessageMetadata } from "@/lib/api";
import { useToast } from "@/hooks/use-toast";
import { Button } from "@/components/ui/button";
import { Skeleton } from "@/components/ui/skeleton";
import { Card } from "@/components/ui/card";
import { Textarea } from "@/components/ui/textarea";
import { ChevronLeft, MessageCircle, MessageSquarePlus } from "lucide-react";
import { useIsMobile } from "@/hooks/use-mobile";
import { cn } from "@/lib/utils";

interface MessengerThread extends MessengerMessageMetadata {}

interface MessengerMessage extends MessengerMessageMetadata {}
...
export default function MessengerChannel() {
  ...
  const loadThreads = async () => {
    try {
      ...
      const response = await messengerApi.listThreads();
      setThreads(response.threads);
    } catch (err) {
      handleApiError(err, "Failed to load Messenger threads");
    } finally {
  ...
  const loadMessages = async (threadId: string) => {
    try {
      ...
      const response = await messengerApi.getThreadMessages(threadId);
      setMessages(response.messages);
    } catch (err) {
      handleApiError(err, "Failed to load Messenger thread messages");
    } finally {
  ...
  const handleCreateTestMessage = async () => {
    try {
      ...
      const message = await messengerApi.createTestInbound();
      toast({
        title: "Test Messenger message created",
        description: "A mock inbound Messenger message has been created for testing.",
      });
      await loadThreads();
      setSelectedThreadId(message.metadata?.messenger_thread_id || null);
      if (message.metadata?.messenger_thread_id) {
        await loadMessages(message.metadata.messenger_thread_id);
      }
    } catch (err) {
      handleApiError(err, "Failed to create test Messenger message");
    } finally {
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const response = await messengerApi.replyToThread(selectedThreadId, replyBody.trim());
      setMessages((prev) => [...prev, response.message]);
      setReplyBody("");
      toast({
        title: "Reply sent",
        description: "Your Messenger reply has been sent.",
      });
    } catch (err) {
      handleApiError(err, "Failed to send Messenger reply");
    } finally {
        ...
        {isMobile && selectedThreadId ? (
          <>
            ...
            <p className="truncate text-sm font-medium text-foreground flex-1 min-w-0">
              {selectedThread?.content || "Messenger Conversation"}
            </p>
          ...
          <>
            <div className="min-w-0">
              ...
              <p className="text-[13px] text-muted-foreground mt-0.5 hidden sm:block">
                Facebook Messenger conversations with customers
              </p>
            </div>
            <Button
            ...
            >
              ...
              {creatingTest ? "Creating…" : "Add test message"}
            </Button>
          </>
        )}
      </div>
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
            {loadingThreads ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                <p className="text-[13px] text-muted-foreground">No Messenger threads yet.</p>
                <p className="text-[12px] text-muted-foreground/80 mt-1">
                  Use &quot;Add test message&quot; to create a sample conversation.
                </p>
                <Button
                  onClick={handleCreateTestMessage}
                  disabled={creatingTest}
                ...
                {threads.map((thread) => (
                  ...
                  >
                    <div className="font-medium text-[13px] sm:text-sm truncate pr-2">
                      {thread.content || "Messenger conversation"}
                    </div>
          ...
          {!selectedThreadId ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-8 text-center min-h-[280px]">
              ...
              <p className="text-[13px] text-muted-foreground mt-1.5 max-w-[260px]">
                Choose a thread from the list to view the Messenger conversation and reply.
              </p>
            </div>
          ) : loadingMessages ? (
            ...
            <>
              ...
              <div className="shrink-0 p-4 sm:p-5 pt-0 border-t border-border/50 space-y-3">
                <label htmlFor="messenger-reply" className="sr-only">
                  Reply
                </label>
                <Textarea
                  id="messenger-reply"
                ...
                <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
                  <p className="text-[12px] text-muted-foreground truncate px-0.5">
                    Reply will be sent as a Messenger message.
                  </p>
                  <Button
