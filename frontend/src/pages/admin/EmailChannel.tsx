...
import { ChevronLeft, RefreshCw, Paperclip } from "lucide-react";
...
export default function EmailChannel() {
  ...
  const loadThreads = async () => {
    try {
      ...
      const response = await emailApi.listThreads();
      setThreads(response.threads);
    } catch (err) {
      handleApiError(err, "Failed to load email threads");
    } finally {
  ...
  const loadMessages = async (threadId: string) => {
    try {
      ...
      const response = await emailApi.getThreadMessages(threadId);
      setMessages(response.messages);
    } catch (err) {
      handleApiError(err, "Failed to load thread messages");
    } finally {
  ...
  const handleSync = async () => {
    try {
      setSyncing(true);
      const result = await emailApi.syncInbox();
      toast({
        title: "Inbox synced",
        description: `Imported ${result.imported} new email message${result.imported === 1 ? "" : "s"}.`,
      });
      await loadThreads();
    } catch (err) {
      handleApiError(err, "Failed to sync email inbox");
    } finally {
      setSyncing(false);
    }
  };
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const response = await emailApi.replyToThread(selectedThreadId, replyBody.trim());
      ...
      handleApiError(err, "Failed to send email reply");
    } finally {
  ...
  return (
    <div className="flex flex-col w-full min-w-0">
      {/* Top bar: title + sync (desktop) or back + title (mobile conversation) */}
        ...
        {isMobile && selectedThreadId ? (
          <>
            ...
            <p className="truncate text-sm font-medium text-foreground flex-1 min-w-0">
              {selectedThread?.content || "Conversation"}
            </p>
          ...
          <>
            <div className="min-w-0">
              <h2 className="text-base font-semibold text-foreground sm:text-lg tracking-tight">Email</h2>
              <p className="text-[13px] text-muted-foreground mt-0.5 hidden sm:block">
                Support inbox threads and replies
              </p>
            </div>
            <Button
              onClick={handleSync}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 min-h-0">
        {/* Thread list: full width on mobile when list visible, else hidden */}
        <Card
          variant="subtle"
          className={cn(
          ...
          <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
            {loadingThreads ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                ...
                <p className="text-[12px] text-muted-foreground/80 mt-1">Sync your inbox to import messages.</p>
                <Button onClick={handleSync} disabled={syncing} variant="outline" size="sm" className="mt-4">
                    ...
                    <div className="flex items-center gap-2 mt-1 text-[12px] text-muted-foreground">
                      ...
                      {thread.metadata?.message_count != null && (
                        <span className="shrink-0">
                          {thread.metadata.message_count} message{thread.metadata.message_count === 1 ? "" : "s"}
                        </span>
                      )}
                    </div>
                  </button>
          ...
          {!selectedThreadId ? (
            <div className="flex-1 flex flex-col items-center justify-center p-6 sm:p-8 text-center min-h-[280px]">
              <div className="rounded-full bg-muted/50 p-4 mb-4">
                <Paperclip className="h-8 w-8 text-muted-foreground/70" aria-hidden />
              </div>
              ...
              <p className="text-[13px] text-muted-foreground mt-1.5 max-w-[260px]">
                Choose a thread from the list to view the conversation and reply.
              </p>
            </div>
          ) : loadingMessages ? (
                      ...
                      >
                        ...
                        >
                          ...
                          {attachments.length > 0 && (
                            <div className="mt-2.5 pt-2.5 border-t border-border/50 opacity-90">
              ...
              <div className="shrink-0 p-4 sm:p-5 pt-0 border-t border-border/50 space-y-3">
                ...
                <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3">
                  <p className="text-[12px] text-muted-foreground truncate px-0.5">
                    Re: {messages[0]?.metadata?.subject || "(no subject)"}
                  </p>
                  <Button
